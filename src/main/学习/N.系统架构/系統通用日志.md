...
* [Springboot-注解-通用操作日志组件](#springboot-注解-通用操作日志组件)
  * [Change Log](#change-log)
  * [使用方式](#使用方式)
    * [基本使用](#基本使用)
      * [maven依赖添加SDK依赖](#maven依赖添加sdk依赖)
      * [SpringBoot入口打开开关,添加 @EnableLogRecord 注解](#springboot入口打开开关添加-enablelogrecord-注解)
      * [日志埋点](#日志埋点)
          * [1. 普通的记录日志](#1-普通的记录日志)
          * [2. 期望记录失败的日志, 如果抛出异常则记录fail的日志，没有抛出记录 success 的日志](#2-期望记录失败的日志-如果抛出异常则记录fail的日志没有抛出记录-success-的日志)
          * [3. 日志支持种类](#3-日志支持种类)
          * [4. 支持记录操作的详情或者额外信息](#4-支持记录操作的详情或者额外信息)
* [order.toString() 是调用了 Order 的 toString() 方法。 如果保存 JSON，自己重写一下 Order 的 toString() 方法就可以。](#ordertostring-是调用了-order-的-tostring-方法-如果保存-json自己重写一下-order-的-tostring-方法就可以)
          * [5. 如何指定操作日志的操作人是什么？ 框架提供了两种方法](#5-如何指定操作日志的操作人是什么-框架提供了两种方法)
          * [6. 日志文案调整](#6-日志文案调整)
          * [7. 日志文案调整 使用 SpEL 三目表达式](#7-日志文案调整-使用-spel-三目表达式)
          * [8. 日志文案调整 模版中使用方法参数之外的变量&函数中也可以使用Context中变量](#8-日志文案调整-模版中使用方法参数之外的变量函数中也可以使用context中变量)
          * [9. 函数中使用LogRecordContext的变量](#9-函数中使用logrecordcontext的变量)
          * [10. 使用 condition，满足条件的时候才记录日志](#10-使用-condition满足条件的时候才记录日志)
      * [框架的扩展点](#框架的扩展点)
      * [变量相关](#变量相关)
      * [Change Log & TODO](#change-log--todo)
      * [注意点：](#注意点)
  * [Author](#author)
