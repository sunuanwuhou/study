...
1. 我们先重构一下之前的 Alert 代码，让它的扩展性更好一些。重构的内容主要包含两部分：

   1. 第一部分是将 check() 函数的多个入参封装成 ApiStatInfo 类；

      > 目前项目也是这中，尽量多用DTO的方式去提高参数的可扩展化。

   2. 第二部分是引入 handler 的概念，将 if 判断逻辑分散在各个 handler 中。具体的代码实现如下所示：

      > 这个倒是没用到，毕竟项目中业务代码还是毕竟复杂，难以抽象化。而且最根本的就是费时间，也不能过度设计。
...
public class Alert {
    ...
    public void check(ApiStatInfo apiStatInfo) {
        for (AlertHandler handler : alertHandlers) {
			//这个就是策略模式的应用了，将数据传递过去，具体实现类自己判断，
