...
* [什么是Reactor](#什么是reactor)
* [Reactor](#reactor)
  * [单 Reactor 单进程 / 线程](#单-reactor-单进程--线程)
  * [单 Reactor 多进程 / 线程](#单-reactor-多进程--线程)
...
Reactor 模式主要由 **Reactor 和处理资源池**这两个核心部分组成，它俩负责的事情如下：
...
![](.images/下载-1632310995846.png)



可以看到进程里有 **Reactor、Acceptor、Handler** 这三个对象：

- Reactor 对象的作用是**监听**和**分发事件**；
...
> 对象里的 `select`、`accept`、`read`、`send` 是**系统调用函数**，dispatch 和 「业务处理」是需要完成的操作，其中 dispatch 是分发事件操作。
...
> 一个Reactor处理所有监听事件和分发，一个Handler处理所有业务。
...
> 这里会有几个Handler？是每个事件都会由一个Handler，然后都有对应的线程池？
> 
...
> 高并发下，单Reactor和多Handler下共享资源的线程安全问题都会成为瓶颈。



## 多 Reactor 多进程 / 线程

> Reactor负责监听和分发，这是可以拆开的。那么拆开后怎么通信呢？

要解决「单 Reactor」的问题，就是将「单 Reactor」实现成「多 Reactor」，这样就产生了第 **多 Reactor 多进程 / 线程**的方案。

老规矩，闻其名不如看其图。多 Reactor 多进程 / 线程方案的示意图如下（以线程为例）：

![](.images/下载-1632313917887.png)
...
> 如何避免多线程竞争共享资源的问题?多Reactor多线程怎么解决的？
...
前面提到的 Reactor 是非阻塞**同步**网络模式，而 **Proactor 是异步网络模式**。
