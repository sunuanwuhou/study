...
![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfxxjvoTicwpUmESgBvvjqH5TR3KGvAsF9IKG9E0myY6CpvO7mVBUWehtlDvOdib6iaoicxelvn4lXMSg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
> 发送方发送了很多个 packet，每个 packet 都有自己的序号，你可以认为是 TCP 的序列号，其中 `packet #3` 在网络中丢失了，即使 `packet #4-6` 被接收方收到后，由于内核中的 TCP 数据不是连续的，于是接收方的应用层就无法从内核中读取到，只有等到 `packet #3` 重传后，接收方的应用层才可以从内核中读取到数据。
