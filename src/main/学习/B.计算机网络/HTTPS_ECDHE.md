...
![](.images/下载-1633609443669.png)

那么对于底数为 2 的时候， 32 的对数是 5，64 的对数是 6，计算过程如下：

![](.images/下载-1633609475812.png)

对数运算的取值是可以连续的，**而离散对数的取值是不能连续的**，因此也以「离散」得名，

离散对数是在**对数运算的基础上加了「模运算」**，也就说取余数，对应编程语言的操作符是「%」，也可以用 mod 表示。离散对数的概念如下图：

![](.images/下载-1633609552874.png)
...
现假设小红和小明约定使用 DH 算法来交换密钥，那么基于离散对数，小红和小明需要先确定**模数**和**底数**作为算法的参数，**这两个参数是公开的，用 P 和 G 来代称。**
...
![](.images/下载-1633610642479.png)

接着，客户端会发「**Encrypted Handshake Message**」消息，把之前发送的数据做一个摘要，再用对称密钥加密一下，让服务端做个验证，验证下本次生成的对称密钥是否可以正常使用。

![](.images/下载-1633610672801.png)
