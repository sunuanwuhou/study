...
* [TLS 第二次握手](#tls-第二次握手)
* [客户端验证证书](#客户端验证证书)
  * [数字证书和 CA 机构](#数字证书和-ca-机构)
  * [数字证书签发和验证流程](#数字证书签发和验证流程)
  * [证书链](#证书链)
* [TLS 第三次握手(重点)](#tls-第三次握手重点)
* [TLS 第四次握手](#tls-第四次握手)
* [RSA 算法的缺陷](#rsa-算法的缺陷)
* [总结](#总结)
...
> 经过TCP三次握手,在经过TLS的四次握手,才能建立通信安全
...
> 如果私钥被窃取,那就泄漏了,这也是缺陷所在.

在 RSA 密钥协商算法中，客户端会生成随机密钥，并使用服务端的公钥加密后再传给服务端。根据非对称加密算法，公钥加密的消息仅能通过私钥解密，这样服务端解密后，双方就得到了相同的密钥，再用它加密应用消息。

我用 Wireshark 工具抓了用 RSA 密钥交换的 TLS 握手过程，你可以从下面看到，一共经历来四次握手：

![](.images/下载-1632379773491.png)

对应 Wireshark 的抓包，我也画了一幅图，你可以从下图很清晰地看到该过程：

![](.images/下载-1632379885372.png)



> Q:使用三个随机数进行对称加密，使用非对称加密进行加密，这个时候服务端是可以用私钥获取非对称加密密文进行解密,数据传输到客户端,
>
> 客户端怎么对传输的数据进行解密得到对称加密的密文呢?客户端是没有私钥的?
>
> A:在第三次握手的时候,客户端会将第三次随机数发给服务端,这样双方就都有了对称加密密钥.
...
![](.images/下载-1632384100122.png)

随后，服务端发了「**Server Hello Done**」消息，目的是告诉客户端，我已经把该给你的东西都给你了，本次打招呼完毕。

![](.images/下载-1632384122396.png)
...
> CA和客户端都会计算数字证书的Hash值,CA用私钥进行加密Hash,客户端用公钥进行解密,比对自己计算出的hash值
...
![]()![下载](.images/下载-1632387034623.png)
...
![](.images/下载-1632387067087.png)

这样的一层层地验证就构成了一条信任链路，整个证书信任链验证流程如下图所示：

![](.images/下载-1632387097161.png)
...
客户端验证完证书后，认为可信则继续往下走。接着，客户端就会生成一个新的**随机数 (\*pre-master\*)**，用服务器的 **RSA 公钥**加密该随机数，通过「**Change Cipher Key Exchange**」消息传给服务端。

![](.images/下载-1632388243142.png)
...
> 图片多看几遍，ecdhe就是一套算法而已

![](.images/下载-1632389771841.png)
