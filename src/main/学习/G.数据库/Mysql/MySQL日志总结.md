...
所以，不同的操作，需要记录的内容也是不同的，所以不同类型的操作（修改、删除、新增）产生的 undo log 的格式也是不同的，具体的每一个操作的 undo log 的格式我就不详细介绍了，感兴趣的可以自己去查查。
...
这两个隔离级别实现是通过「事务的 Read View 里的字段」和「记录中的两个隐藏列（trx_id 和 roll_pointer）」的比对，如果不满足可见行，就会顺着 undo log 版本链里找到满足其可见性的记录，从而控制并发事务访问同一个记录时的行为，这就叫 MVCC（多版本并发控制）。具体的实现可以看我这篇文章：[事务隔离级别是怎么实现的？](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247508002&idx=1&sn=9a59db214082ca8810dcdcb4af43c17c&scene=21#wechat_redirect)
...
> 小知识：
>
> 因为MySQL事务执行过程中产生的Undo Log也需要进行持久化操作，所以Undo Log也会产生Redo Log。由于Undo Log的完整性和可靠性需要Redo Log来保证，因此数据库崩溃时需要先做Redo Log数据恢复，然后做Undo Log回滚。
...
- 当修改数据时，如果数据存在于  Buffer Pool  中，那直接修改 Buffer Pool 中数据所在的页，然后将其页设置为脏页（该页的内存数据和磁盘上的数据已经不一致），为了减少磁盘I/O，**不会立即将脏页写入磁盘，后续由后台线程选择一个合适的时机将脏页写入到磁盘。**
  ...
  > 这里我最开始就理解错了，
  >
  > 是先写redo日志，日志刷盘，脏页后刷入磁盘
  >
  > 数据写日志和数据写磁盘是2回事，千万不要搞混了！！

之前专门写过一篇介绍 Buffer Pool 的文章：[揭开 Buffer Pool 的面纱](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247507629&idx=1&sn=ed9bc5efb0faef11c59a74e579129158&scene=21#wechat_redirect)，所以这里就不做太多的介绍。
...
0：延迟写，实时刷。提交事务时不会将redo log写入os buffer，而是每隔1秒将redo log写入os buffer并调用fsync()刷入磁盘。系统崩溃会丢失一秒钟的数据。
...
将数据页中的LSN值和Redo Log中的LSN值进行比较，**如果数据页中的LSN值小于Redo Log中的LSN值**，则表示丢失了一部分数据，此时，可以通过Redo Log的记录来恢复数据，否则不需要恢复数据。



# **为什么需要 binlog ？server层**
...
5. 开启事务， InnoDB 层更新记录前，首先要记录相应的 undo log，因为这是更新操作，需要把被更新的列的旧值记下来，也就是要生成一条 undo log，undo log 会写入 Buffer Pool 中的 Undo 页面，不过在修改该 Undo 页面前需要先记录对应的 redo log，所以**先记录修改 Undo 页面的 redo log ，然后再真正的修改 Undo 页面**。

6. InnoDB 层开始更新记录，根据 WAL 技术，**先记录修改数据页面的 redo log ，然后再真正的修改数据页面**。修改数据页面的过程是修改 Buffer Pool 中数据所在的页，然后将其页设置为脏页，为了减少磁盘I/O，不会立即将脏页写入磁盘，后续由后台线程选择一个合适的时机将脏页写入到磁盘。

7. 至此，一条记录更新完了。

8. 在一条更新语句执行完成后，然后开始记录该语句对应的 binlog，此时记录的 binlog 会被保存到 binlog cache，并没有刷新到硬盘上的 binlog 文件，在事务提交时才会统一将该事务运行过程中的所有 binlog 刷新到硬盘。

9. 事务提交，剩下的就是「两阶段提交」的事情了，接下来就讲这个。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8Lwrm0XzoSdcncWM16VqLL1ibibibpXibR6BiaTpCRyFuejLvpobOt9Wu0psA/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)每个阶段都有一个队列
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8L5Zrbw9sOg1r879tPsu4zZibUZQ7INf7WKUWPJmzM0aM2jwbb21USMoA/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

接着，获取队列中的事务组，由绿色事务组的 Leader 对 rodo log 做一次  write + fsync，即一次将同组事务的 redolog 刷盘：

![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8LvdE3zOkPSNYvVcXAhD4piboB8FpnnsiahC7ghqCvvTdu2C3Pc7ickm8FQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8LIau2fIdPz3KCeWP2mlz77EXZn9BSdYmM6mNK6RhDnV0Kib0II3QFQOw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

不过，在等待的过程中，如果事务的数量提前达到了 `Binlog_group_commit_sync_no_delay_count` 参数设置的值，就不用继续等待了，就马上将 binlog 刷盘，如下图：

![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8Llr3OHVJUnpW6QFxicy9AulzKm8GOVlXdibsDy0y3rwh1WrWJwMaaJUbg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZfbMTxAxc50c4vc9fiaxgk8LG0Ya9fRyjOIWWf9q11tCMcdibht3ic1xTXXYxJxNW2J7g2OtWX8dIgYg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
现在我们知道事务在提交的时候，需要将 binlog 和 redo log 持久化到磁盘。

那么如果出现 MySQL 磁盘 I/O 很高的现象，我们可以通过控制以下参数，来 “延迟” binlog 和 redo log 刷盘的时机，从而降低磁盘 I/O 的频率：
...
9. 事务提交（为了方便说明，这里不说组提交的过程，只说两阶段提交）：

10. - **prepare 阶段**：将 redo log 对应的事务状态设置为 prepare，然后将 redo log 刷新到硬盘；
    - **commit 阶段**：将 binlog 刷新到磁盘，接着调用引擎的提交事务接口，将 redo log 状态设置为commit；

11. 至此，一条更新语句执行完成。

# 参考资料

https://mp.weixin.qq.com/s/tDVd8oJ9U3lBThnGybbhFw?forceh5=1
