...
滤完成之后，计算参与join的各个字段的总数据量，数据量小的那个表，就是“小表”，应该
作为驱动表。
...
4. 小表驱动大表。
    ...
    + `Block Nested-Loop Join`： 不管谁做驱动表，本身扫描总行数是一样的，但是buffer有大小。驱动表越大，会将数据分成多次，加载到buffer中，会导致被驱动表反复被加载。**内存判断次数是不受选择哪个表作为驱动表影响的**
